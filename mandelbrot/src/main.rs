...
fn main() {
    ...
    // println!("cores {}; physical cores {}", cores, physical_cores);
...
/// the character given by the separator argument, and the <left> and <right> are
...
/// corresponding point in the complexplane.
///
...
fn pixel_to_point(
    ...
    Complex {
        // distance from the left + pixel position * resolution of the image
        re: upper_left.re + pixel.0 as f64 * width / bounds.0 as f64,
        // Why substraction here? pixel.1 increases as we go down,
        // but the imaginary component increases as we go up.
        im: upper_left.im - pixel.1 as f64 * height / bounds.1 as f64,
        // im: lower_right.im + (bounds.1 - pixel.1) as f64 * height / bounds.1 as f64,
    }
}
    ...
    for row in 0..bounds.1 {
        for column in 0..bounds.0 {
            ...
            pixels[row * bounds.0 + column] = match escape_time(point, 255) {
                // Point is in Mandelbrot set, it didn't escape within the 255 iterations limit.
                // We'll color its pixel black.
                None => 0,
                // Point is not in the set and it took count iterations to find out.
                // We want to color with darker pixels the points that took more iterations
                // to figure out they're not in the set which is why we do 255 - count since
                // a smaller value gives a darker pixel.
